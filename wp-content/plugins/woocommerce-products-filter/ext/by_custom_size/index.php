...
//24-04-2017
final class WOOF_EXT_BY_CUSTOM_SIZE extends WOOF_EXT {
	
    public $type = 'by_html_type';
    public $html_type = 'by_custom_size'; //your custom key here
    public $index = 'woof_custom_size';
	...
	add_action('wp_ajax_woof_custom_size_autocomplete', array($this, 'woof_custom_size_autocomplete'));
	add_action('wp_ajax_nopriv_woof_custom_size_autocomplete', array($this, 'woof_custom_size_autocomplete'));
	...
	self::$includes['js_init_functions'][$this->html_type] = 'woof_init_custom_size'; //we have no init function in this case*/
	//***
	add_shortcode('woof_custom_size_filter', array($this, 'woof_custom_size_filter')); 
    }
	...
	if (isset($request['s'])) {
	    ...
	    $request['woof_custom_size'] = $request['s'];
	    //unset($request['s']);
	}
	...
	global $WOOF;
	//***
	//var_dump($WOOF->settings['by_custom_size']);
	if (isset($WOOF->settings['by_custom_size']['autocomplete']) AND $WOOF->settings['by_custom_size']['autocomplete']) {
	...
	<style type="text/css">
	<?php
	if (isset($WOOF->settings['by_custom_size']['image'])) {
	    if (!empty($WOOF->settings['by_custom_size']['image'])) {
		?>
		    .woof_custom_size_search_container .woof_custom_size_search_go{
			background: url(<?php echo $WOOF->settings['by_custom_size']['image'] ?>) !important;
		    }
		<?php
	    }
	}
	...
	<script type="text/javascript">
	    ...
	    woof_lang_custom.<?php echo $this->index ?> = "<?php _e('By size', 'woocommerce-products-filter') ?>";

	    var woof_custom_size_autocomplete = 0;
	    var woof_custom_size_autocomplete_items = 10;
	<?php if (isset($WOOF->settings['by_custom_size']['autocomplete'])): ?>
	        woof_custom_size_autocomplete =<?php echo (int) $WOOF->settings['by_custom_size']['autocomplete']; ?>;
	        woof_custom_size_autocomplete_items =<?php echo apply_filters('woof_custom_size_autocomplete_items', 10) ?>;
	<?php endif; ?>

	    var woof_post_links_in_autocomplete = 0;
	<?php if (isset($WOOF->settings['by_custom_size']['post_links_in_autocomplete'])): ?>
	        woof_post_links_in_autocomplete =<?php echo (int) $WOOF->settings['by_custom_size']['post_links_in_autocomplete']; ?>;
	<?php endif; ?>

	    var how_to_open_links = 0;
	<?php if (isset($WOOF->settings['by_custom_size']['how_to_open_links'])): ?>
	        how_to_open_links =<?php echo (int) $WOOF->settings['by_custom_size']['how_to_open_links']; ?>;
	<?php endif; ?>
    ...
    //shortcode
    public function woof_custom_size_filter($args = array()) {
	global $WOOF;
	$args['loader_img'] = $this->get_ext_link() . 'img/loader.gif';
	return $WOOF->render_html($this->get_ext_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_custom_size_filter.php', $args);
    }
    ...
    public function assemble_query_params(&$meta_query, $wp_query = NULL) {
        ...
        if (isset($request['woof_custom_size']))
        {
            $meta_query[] = array(
                'key' => 'wc_custom_size',
                'value' => $request['woof_custom_size'],
                //'type' => 'CHAR',
                //'compare' => 'BETWEEN'
            );
        }                       
	...
	if (defined('DOING_AJAX')) {
		...
		//$where .= " AND ( meta_key = 'wc_custom_size' AND meta_value = " . $woof_custom_size . " )";

        if (isset($request['woof_custom_size'])) {
		       $meta_query[] = array(
                    array(
                        'key' => 'wc_custom_size',
                        'value' => $woof_custom_size, //instock,outofstock
                        //'compare' => 'IN'
                    )
                );
        }
    ...
    //ajax
    public function woof_custom_size_autocomplete() {

		$_GET['woof_custom_size'] = $_REQUEST['phrase'];
	$meta_query[] = array(
                    array(
                        'key' => 'wc_custom_size',
	...
	$args = array(
	    ...
	    'orderby' => 'title',
	    'order' => 'ASC',
	    'max_num_pages' => intval($_REQUEST['auto_res_count']) > 0 ? intval($_REQUEST['auto_res_count']) : apply_filters('woof_custom_size_autocomplete_items', 10)
	);
...
WOOF_EXT::$includes['html_type_objects']['by_custom_size'] = new WOOF_EXT_BY_CUSTOM_SIZE();
