...
// Add custom fields meta box action...
function woocommerce_custom_fields_data() {

	global $post;

	$breeder = get_post_meta( $post->ID, 'wc_custom_breeder', 'TRUE');
	if($breeder) {
		echo '<div class="custom_field_row row_breeder">
		  <b class="label">Breeder: </b><span>' . $breeder . '</span>
		</div>';
	}

	$sex = get_post_meta( $post->ID, 'wc_custom_sex', 'TRUE');
	if($sex) {
		echo '<div class="custom_field_row row_sex">
		  <b span="label">Sex: </b><span>' . ucfirst($sex) . '</span>
		</div>';
	}

	$born = get_post_meta( $post->ID, 'wc_custom_born_in', 'TRUE');
	if($born) {
		echo '<div class="custom_field_row row_born">
		  <b class="label">Born in: </b><span>' . $born . '</span>
		</div>';
	}

	$size = get_post_meta( $post->ID, 'wc_custom_size', 'TRUE');
	if($size) {
		echo '<div class="custom_field_row row_size">
		  <b class="label">Size: </b><span>' . $size . '</span>
		</div>';
	}

	$variety = get_post_meta( $post->ID, 'wc_custom_variety', 'TRUE');
	if($variety) {
		echo '<div class="custom_field_row row_variety">
		  <b class="label">Variety: </b><span>' . $variety . '</span>
		</div>';
	}

	$estimate = get_post_meta( $post->ID, 'wc_custom_estimate_time', 'TRUE');
	if($estimate) {
		echo '<div class="custom_field_row row_estimate">
		  <b class="label">Estimated Value: </b><span>' . $estimate . '</span>
		</div>';
	}
}

add_action( 'woocommerce_get_custom_fields_data', 'woocommerce_custom_fields_data');
